...
function replaceKnji(a) {
  a && (a = a.replace("601", "六〇一"), a = a.replace("634", "六三四"), a = a.replace("931", "九三一"), a = a.replace("22", "二二"), a = a.replace("34", "三四"), a = a.replace("96", "九六"), a = a.replace("97", "九七"), a = a.replace("99", "九九"));
  return a
}
...
function initialize(a) {
  ...
  if (setting.version !== c.id) {
    ...
    for (var e = d.next(); !e.done; e = d.next()) {
      ...
      h.textContent = 0 === e.type ? "新規" : 1 === e.type ? "変更" : "修正";
  ...
  for (d = 0; d < b.length; d++) e = b[d], e.classList.add("lb_info_lb_" + (Math.floor(d / 4) + 1)), e = e.getElementsByClassName("info_name"), e.length && (e[0].textContent = "第" + (Math.floor(d / 4) + 1) + "基地航空隊");
  ...
  $(".progress_area").each(function (a, b) {
    ...
    6 > a && $(b).find(".progress_label").text("基地" + (Math.floor(a / 2) + 1) + " " + (a % 2 + 1) + "波目");
    6 === a && $(b).find(".progress_label").text("本隊");
    7 === a && $(b).find(".progress_label").text("防空")
  });
  f = e = d = b = "";
  for (g = 1; 10 >= g; g++) b += '<td class="td_battle battle' + g + '">' + g + "戦目</td>", d += '<td class="td_battle battle' + g + ' fap"></td>', e += '<td class="td_battle battle' + g + ' eap"></td>', f += '<td class="td_battle battle' + g + ' cond"></td>';
  $("#shoot_down_table").find(".tr_header").append(b + '<td class="td_battle battle_end">出撃後</td><td class="td_battle battle_death">全滅率</td>');
  ...
  $(".tr_cond").append(f);
  b = "";
  for (d = 1; 10 >= d; d++) b += '\n\t\t\t<li class="nav-item ' + (1 === d ? "" : "d-none") + '">\n\t\t\t\t<a class="nav-link ' + ("#000000" === mainColor ? "" : "nav-link-dark") + " " + (1 === d ? "active" : "") + '" data-toggle="tab" data-disp="' + d + '" href="#">' + d + "戦目</a>\n\t\t\t</li>";
  ...
  $("#rate_table tbody").find("tr").each(function (a, b) {
    ...
    6 > a ? ($(b).find(".rate_td_name").html('<span class="mr-1">基地' + c + '</span><span class="text-nowrap">' + d + "波目</span>"), $(b)[0].dataset.base_no = c - 1, $(b).addClass("cur_pointer land_base_detail")) : 6 === a ? $(b).find(".rate_td_name").text("本隊") : 7 === a && $(b).find(".rate_td_name").text("防空");
  ...
  for (e = d.next(); !e.done; e = d.next()) e = e.value, $("#init_prof_parent").append(b), f = $("#init_prof_parent").find(".type_name:last"), f.text(e.abbr), f.closest(".init_prof")[0].dataset.typeid = e.id;
  $("#init_prof_parent").find(".init_prof:first").remove();
  b = $jscomp.makeIterator(setting.defaultProf);
  for (e = b.next(); !e.done; e = b.next()) d = e.value, proficiency_Changed($('.init_prof[data-typeid="' + d.id + '"').find('.prof_opt[data-prof="' + d.prof + '"]').parent(), !0);
  if (setting.contentsOrder.length) {
    b = $("#main_contents");
    d = $("#side_index");
    var m = [],
      l = [];
    $(".contents").each(function (a, b) {
      m.push($(b))
    });
    b.empty();
    ...
    d.empty();
    e = {};
    f = $jscomp.makeIterator(setting.contentsOrder);
    for (g = f.next(); !g.done; e = {
        $jscomp$loop$prop$id$134$302: e.$jscomp$loop$prop$id$134$302
      }, g = f.next()) e.$jscomp$loop$prop$id$134$302 = g.value, g = m.find(function (a) {
      return function (b) {
        return b.attr("id") === a.$jscomp$loop$prop$id$134$302
      }
    }(e)), b.append(g), g = l.find(function (a) {
      return function (b) {
        return b.attr("id") === "li_" + a.$jscomp$loop$prop$id$134$302
      }
    }(e)), d.append(g)
  }
  loadMainPreset();
  ...
  setting.simulateCount ? $("#calculate_count").val(setting.simulateCount) : $("#calculate_count").val(5E3);
  setting.initialProf120 && (initialProf120Plane = setting.initialProf120);
  b = $jscomp.makeIterator(initialProf120Plane);
  for (e = b.next(); !e.done; e = b.next()) $("#prof120_" + e.value).prop("checked", !0);
  setting.displayMode ? Object.keys(setting.displayMode).forEach(function (a) {
    $("#" + a + ' .toggle_display_type[data-mode="' + setting.displayMode[a] + '"]').addClass("selected")
  }) : $('.toggle_display_type[data-mode="single"]').addClass("selected");
  setting.orderBy || (setting.orderBy = {
    modal_plane_select: ["default", "desc"]
  });
  Object.keys(setting.orderBy).forEach(function (a) {
    $("#" + a + " .sort_select").val(setting.orderBy[a][0]);
    $("#" + a + " .order_" + setting.orderBy[a][1]).prop("checked", !0)
  });
  $("#auto_save").prop("checked", setting.autoSave);
  $("#backup_enabled").prop("checked", setting.backUpEnabled);
  $("#suspend_backup").prop("checked", !1);
  ...
  $("#enemy_word").val("");
  $("#stock_word").val("");
  $("#modal_auto_expand").find(".custom-control-input").prop("checked", !1);
  $("#btn_start_auto_expand").prop("disabled", !0);
  $("#mode_1").prop("checked", !0);
  $("#dest_ap").val(100);
  $("#dest_range").val(6);
  "none" === $("#lb_tab_select").css("display") || $("#lb_item1").attr("class").includes("tab-pane") || ($(".lb_tab").addClass("tab-pane fade"), $(".baseNo").removeClass("cur_move sortable_handle"), $(".lb_tab:first").addClass("show active"), $("#lb_item1").addClass("active"));
  b = document.createDocumentFragment();
  d = 0;
  e = $jscomp.makeIterator(CHANGE_LOG);
  for (f = e.next(); !f.done; f = e.next())
    for (f = f.value, g = document.createElement("div"), g.className = "my-3 ver_log border-bottom", h = document.createElement("div"), h.className = "py-2", h.textContent = "v" + f.id, c.id === f.id && (k = document.createElement("span"), k.className = "ml-2 pt-1 badge badge-pill badge-danger", k.textContent = "New", h.appendChild(k)), g.appendChild(h), h = 0; h < f.changes.length; h++) {
      k = f.changes[h];
      var n = "log_" + d++ + "_" + h,
        u = document.createElement("div");
      u.className = "py-2 px-2 d-flex collapsed history_item history_item_no_content";
      var p = document.createElement("div");
      p.className = "d-flex flex-nowrap align-self-center";
      var q = document.createElement("div");
      q.className = "align-self-center";
      var r = document.createElement("span");
      r.className = "mr-2 pt-1 badge badge-pill badge-" + (0 === k.type ? "success" : 1 === k.type ? "info" : "danger");
      r.textContent = 0 === k.type ? "新規" : 1 === k.type ? "変更" : "修正";
      var t = document.createElement("div");
      t.className = "align-self-center";
      t.innerHTML = k.title;
      q.appendChild(r);
      p.appendChild(q);
      p.appendChild(t);
      u.appendChild(p);
      g.appendChild(u);
      k.content && (u.dataset.toggle = "collapse", u.dataset.target = "#" + n, u.classList.remove("history_item_no_content"), u = document.createElement("div"), u.id = n, u.className = "border-top collapse history_content", n = document.createElement("div"), n.className = "pl-3 py-2 font_size_12", n.innerHTML = k.content, u.appendChild(n), g.appendChild(u));
      b.appendChild(g)
    }
  c = document.createElement("div");
  c.className = "mt-2 font_size_12";
  c.textContent = LAST_UPDATE_DATE;
  b.appendChild(c);
  document.getElementById("site_history_body").appendChild(b);
  "dark" === setting.themeColor ? (document.getElementById("normal_theme").checked = !1, document.getElementById("dark_theme").checked = !0, document.getElementById("dark_gradient_theme").checked = !1, document.getElementById("deep_blue_theme").checked = !1) : "dark-gradient" === setting.themeColor ? (document.getElementById("normal_theme").checked = !1, document.getElementById("dark_theme").checked = !1, document.getElementById("dark_gradient_theme").checked = !0, document.getElementById("deep_blue_theme").checked = !1) : "deep-blue" === setting.themeColor ? (document.getElementById("normal_theme").checked = !1, document.getElementById("dark_theme").checked = !1, document.getElementById("dark_gradient_theme").checked = !1, document.getElementById("deep_blue_theme").checked = !0) : (document.getElementById("normal_theme").checked = !0, document.getElementById("dark_theme").checked = !1, document.getElementById("dark_gradient_theme").checked = !1, document.getElementById("deep_blue_theme").checked = !1);
  site_theme_Changed(!1);
  $('[data-toggle="tooltip"]').tooltip();
  $("#modal_version_inform").find(".modal-body").html() && $("#modal_version_inform").modal("show");
  a()
}
...
function clearPlaneDiv(a) {
  ...
  a.find(".plane_name_span").text("機体を選択");
...
function clearShipDiv(a) {
  ...
  a.find(".ship_name_span").text("艦娘を選択");
...
function clearEnemyDiv(a) {
  ...
  a.find(".enemy_name_text").text("敵艦を選択");
...
function drawEnemyGradeColor(a) {
  a.includes("elite") ? a = a.replace("elite", '<span class="text-danger">elite</span>') : a.includes("改flagship") ? a = a.replace("flagship", '<span class="text-primary">flagship</span>') : a.includes("flagship") && (a = a.replace("flagship", '<span class="text-warning">flagship</span>'));
  return a
}
...
function createPlaneTable(a) {
  ...
  for (var n = 0, u = 0, p = 0, q = 0, r = {}, t = 0; t < b; r = {
    ...
    if (!m || setting.favoritePlane.includes(r.$jscomp$loop$prop$plane$306.id)) {
      ...
      v && (C.dataset.toggle = "tooltip", C.title = "出撃時:" + w + " , 防空時:" + y);
      switch (l) {
...
function loadPlanePreset() {
  ...
  a.find(".preset_name").val("左のプリセット一覧をクリック").prop("disabled", !0);
...
function createNodeSelect() {
  ...
  c.sort(function (a, b) {
    return "空襲" == a.node && "空襲" == b.node ? 0 : "空襲" == a.node ? -1 : "空襲" == b.node ? 1 : 0
  });
  ...
  $("#enemy_pattern_select").html('<li class="nav-item"><a class="nav-link ' + ("#000000" === mainColor ? "" : "nav-link-dark") + ' active" data-toggle="tab" href="#">編成1</a></li>');
...
function expandEnemy() {
  ...
  isDefMode || "空襲" !== b ? isDefMode && "空襲" !== b && (isDefMode = !1, $(".ohuda_select").each(function (a, b) {
...
function drawMainPreset(a) {
  ...
  if (a && 0 < a[0]) b.find(".preset_data").data("presetid", a[0]), b.find(".preset_name").val(a[1]), b.find("#preset_remarks").val(a[3]), b.find(".btn_commit_preset").text("編成変更"), b.find(".btn_commit_preset").prop("disabled", !1), b.find(".btn_commit_preset_header").removeClass("d-none"), b.find(".btn_commit_preset_header").prop("disabled", !1);
  else if (a && 0 > a[0]) {
    ...
    b.find(".btn_commit_preset").text("プリセット保存");
    ...
    b.find(".btn_commit_preset").text("新規登録");
    b.find(".btn_commit_preset").prop("disabled", !0);
    b.find(".btn_commit_preset_header").addClass("d-none");
...
function updateMainPreset() {
  ...
  $("#modal_main_preset").find(".task").text(b ? "編成更新" : "新規登録");
...
function calculateInit(a) {
  ...
  if (document.getElementById("auto_save").checked && (saveLocalStorage("autoSaveData", d), setting.autoSave = !0, saveLocalStorage("setting", setting), document.getElementById("backup_enabled").checked && !document.getElementById("suspend_backup").checked)) {
    ...
    a = [0, "backup-" + a.join("") + b.join(""), d, a.join("/") + " " + b.join(":") + "の編成"];
          ...
          for (var y = 0; y < c.length; y++) {
            ...
            if (q || 0 !== v.id && 0 !== v.slot) {
              ...
              if (!q || !w || 0 !== v.id && 0 !== v.slot) {
                ...
                t && (A = document.createElement("td"), A.rowSpan = u, A.className = "info_name", A.textContent = q ? q.name : "未指定", z.appendChild(A));
                A = document.createElement("td");
...
function updateEnemyFleetInfo(a, b) {
  ...
  if (b) {
    isDefMode || (g.includes("999-") ? document.getElementById("route").value = "海域未指定" : document.getElementById("route").value = k ? "別海域のセルが混在しています。" : g + "：" + h);
    ...
    g.childNodes.length || (h = document.createElement("tr"), k = document.createElement("td"), k.colSpan = 6, k.className = "font_size_11", k.textContent = "戦闘機、攻撃機を搭載した敵艦のいない戦闘です。", h.appendChild(k), g.appendChild(h));
  ...
  updateDetailChart(m, "残機数 [機]", {
    ...
    callbacks: {
      title: function (a, b) {
        ...
        return "残機数：" + c + " 機\n航空戦火力：" + (Math.floor(u[0] * (p * Math.sqrt(c) + 25)) + (d ? " or " + d : ""))
      },
      label: function (a, b) {
        return [a.yLabel + " %"]
      }
...
function landBaseDetailCalculate(a, b) {
  ...
  updateDetailChart(g, "残機数 [機]", {
    mode: "index",
    callbacks: {
      title: function (a, b) {
        var c = a[0].xLabel,
          d = "航空戦火力：" + (getLandBasePower(A, c, C) * F * D).toFixed(1);
        return c ? "残機数：" + c + " 機\n" + d : "残機数：0 機\n航空戦火力：0"
      },
      label: function (a, b) {
        return [a.yLabel + " %"]
  ...
  }(g)), e += '\n\t\t<div class="row mt-0_5 ' + (b === d ? "text-primary" : "") + '">\n\t\t\t<div class="col-2 align-self-center text-right">\n\t\t\t\t<div class="btn_show_detail py-0_5 ' + (b === d ? "selected" : "") + " " + (f ? "" : "disabled") + '" data-slot_no="' + d + '">表示</div>\n\t\t\t</div>\n\t\t\t<div class="col d-flex align-self-center font_size_12 pl-0">\n\t\t\t\t<img src="./img/type/' + (f ? "type" + f.type : "undefined") + '.png" class="plane_img_sm align-self-center">\n\t\t\t\t<div class="align-self-center">' + (f ? f.abbr ? f.abbr : f.name : "-") + '</div>\n\t\t\t</div>\n\t\t\t<div class="col font_size_11 align-self-center">\n\t\t\t\t' + (f ? "(搭載: " + g.$jscomp$loop$prop$pl$347.slot + "機" + (f.torpedo ? "　雷装: " + f.torpedo : "") + (f.bomber ? "　爆装: " + f.bomber : "") + ")" : "") + "\n\t\t\t</div>\n\t\t</div>", d++;
  $("#detail_legend").html("<span>第" + (a + 1) + "基地航空隊</span>");
  $("#detail_info").html(e)
}
...
function enemySlotDetailCalculate(a, b) {
  ...
  updateDetailChart(k, n ? "残機数 [機]" : "航空戦火力", {
    mode: "index",
    callbacks: {
      title: function (a, b) {
        var c = a[0].xLabel;
...
function btn_ex_setting_Clicked(a) {
  ...
  switch (a) {
  case "landBase":
    b.find(".modal-title").text("一括設定　-基地航空隊-");
    ...
    break;
  case "friendFleet":
    b.find(".modal-title").text("一括設定　-艦娘-"),
...
function enemy_ap_select_parent_Show(a) {
  ...
  a.find(".dropdown-header").text(-1 !== b ? "敵艦に「直接入力」を指定した場合編集できます。" : "制空値を入力");
...
function btn_continue_expand_Clicked() {
  ...
  expandEnemy();
  $("#enemy_pattern_tbody").html("<div>" + a + " 戦目の敵艦隊が挿入されました。</div><div>続ける場合は再度戦闘マスを選択してください。<div>");
  $("#enemy_pattern_select").html('<li class="nav-item"><a class="nav-link ' + ("#000000" === mainColor ? "" : "nav-link-dark") + ' active" data-toggle="tab" href="#">編成1</a></li>');
  $("#enemy_pattern_air_power").text("0");
  $("#enemy_display_mode_parent").addClass("d-none").removeClass("d-flex");
...
function btn_load_equipment_json_Clicked() {
  ...
  readEquipmentJson(a) ? (setPlaneStockTable(), $("#input_equipment_json").val(""), $("#input_equipment_json").removeClass("is-invalid").addClass("is-valid")) : ($("#input_equipment_json").addClass("is-invalid").removeClass("is-valid"), $("#input_equipment_json").nextAll(".invalid-feedback").text("装備情報の読み込みに失敗しました。入力されたデータ形式が正しくない可能性があります。"))
}

function input_equipment_json_Changed(a) {
  var b = a.val().trim(),
    c = $("#btn_load_equipment_json");
  0 === b.length ? (a.addClass("is-invalid").removeClass("is-valid"), a.nextAll(".invalid-feedback").text("データが入力されていません。"), c.prop("disabled", !0)) : (a.removeClass("is-invalid"), c.prop("disabled", !1))
}
...
function stock_tr_Clicked(a) {
  ...
  f ? (c.text(f.name), c.attr("title", "割合撃墜補正：" + f.adj[0] + ", 固定撃墜補正：" + f.adj[1])) : (c.text("なし"), c.attr("title", "割合撃墜補正：1.0, 固定撃墜補正：1.0"));
  ...
  return $jscomp.asyncExecutePromiseGeneratorProgram(function (e) {
    if (1 == e.nextAddress) {
      ...
      return c ? c.match(/^(https?|ftp)(:\/\/[-_.!~*'()a-zA-Z0-9;\/?:@&=+\$,%#]+)$/) ? c.match(/^(https:\/\/aircalc.page.link\/)([.!~*'()a-zA-Z0-9;\/?:@&=+\$,%#]+)$/) ? e.jumpTo(2) : e.yield(postURLData(c).then(function (a) {
        a.error || !a.shortLink ? (b.value = "", b.placeholder = "短縮に失敗しました") : (b.value = a.shortLink, b.placeholder = "")
      })["catch"](function (a) {
        b.value = "";
        b.placeholder = "短縮に失敗しました"
      }), 2) : (b.value = "", (d = readDeckBuilder(c)) ? (expandMainPreset(d, 0 < d[0][0].length, !0, !1), calculate(), b.placeholder = "編成の反映に成功しました。") : readEquipmentJson(c) ? (setPlaneStockTable(), calculate(), b.placeholder = "所持装備の反映に成功しました。") : b.placeholder = "URLが不正です。", e.jumpTo(2)) : (b.value = "", b.placeholder = "URLを入力してください。", e.jumpTo(2))
    }
    a.textContent = "URL短縮";
...
function output_data_Clicked(a) {
  a.hasClass("is-valid") && copyInputTextToClipboard(a) && a.nextAll(".valid-feedback").text("クリップボードにコピーしました。")
}
...
function btn_load_deck_Clicked() {
  ...
  (a = readDeckBuilder(a)) ? (expandMainPreset(a, 0 < a[0][0].length, !0, !1), $("#input_deck").removeClass("is-invalid").addClass("is-valid")) : ($("#input_deck").addClass("is-invalid").removeClass("is-valid"), $("#input_deck").nextAll(".invalid-feedback").text("編成の読み込みに失敗しました。入力されたデータの形式が正しくない可能性があります。"))
}
...
function btn_output_url_Clicked() {
  ...
  return $jscomp.asyncExecutePromiseGeneratorProgram(function (d) {
    ...
    if (2 != d.nextAddress) return a.val(c), a.nextAll(".valid-feedback").text("生成しました。上記URLをクリックするとクリップボードにコピーされます。"),
...
function btn_output_deck_Clicked() {
  var a = convertToDeckBuilder(),
    b = $("#output_deck");
  $("#output_deck").val(a);
  a ? (b.nextAll(".valid-feedback").text("生成しました。上記文字列をクリックするとクリップボードにコピーされます。"), b.removeClass("is-invalid").addClass("is-valid")) : b.addClass("is-invalid").removeClass("is-valid")
}
...
function btn_start_auto_expand_Clicked() {
  $("#auto_land_base_1").prop("checked") || $("#auto_land_base_2").prop("checked") || $("#auto_land_base_3").prop("checked") || $("#auto_fleet").prop("checked") ? (autoExpand(), $("#alert_auto_expand").text("おまかせ艦載機配備が完了しました。"), $("#alert_auto_expand").addClass("alert-success").removeClass("alert-danger d-none")) : ($("#alert_auto_expand").text("配備対象を選択してください。"), $("#alert_auto_expand").removeClass("alert-success d-none").addClass("alert-danger"))
}
  ...
  Sortable.create($("#lb_tab_parent")[0], {
    ...
    onEnd: function () {
      $(".lb_tab").each(function (a, b) {
        ...
        $(b).find(".baseNo").text("第" + (a + 1) + "基地航空隊")
      });
      calculate()
    }
  });
